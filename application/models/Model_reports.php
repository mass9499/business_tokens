...
class Model_reports extends CI_Model
{
	...
	{
// 		$sql = "SELECT * FROM token_table WHERE invoice_gen_status = ?";
		$sql = "SELECT * FROM token_table WHERE invoice_gen_status = 1 AND token_status = 1";
		$query = $this->db->query($sql, array(1));
		$result = $query->result_array();
		
		$return_data = array();
		foreach ($result as $k => $v) {
			$date = date('Y', strtotime($v['invoice_created_at']));
			$return_data[] = $date;
		}
	...
	{	
		if($year) {
			...
			foreach ($months as $month_k => $month_y) {
				...
				foreach ($result as $k => $v) {
					$month_year = date('Y-m', strtotime($v['invoice_created_at']));
